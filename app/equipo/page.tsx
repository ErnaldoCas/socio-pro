                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="border border-gray-200 text-gray-500 rounded-lg py-2 text-sm"
                    >
                      Cancelar
                    </button>
                    <button
                  ...
                  <div key={c.id} className="p-4">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => setEditando(editando?.id === c.id ? null : c)}
                          className="text-xs text-blue-600 border border-blue-100 bg-blue-50 px-2 py-1 rounded-lg"
                        >
                          Permisos
                        </button>
                        <button
                          onClick={() => eliminar(c.id)}
                          className="text-xs text-red-500 border border-red-100 bg-red-50 px-2 py-1 rounded-lg"
                        >
                          Quitar
                        </button>
                      </div>
                    ...
                    {editando?.id === c.id && (
                      <div className="mt-3 bg-gray-50 rounded-xl p-3">
                            ...
                            <label key={key} className="flex items-center justify-between py-0.5">
                              ...
                              <input
                                ...
                                onChange={e => setEditando({
                                  ...editando,
                                  permisos: { ...editando.permisos, [key]: e.target.checked }
                                })}
                                className="w-4 h-4 accent-green-600"
                              />
                            </label>
                        ...
                        <button
                          onClick={() => actualizarPermisos(c.id, editando.permisos)}
                          className="w-full mt-3 bg-green-600 text-white rounded-lg py-2 text-xs font-medium hover:bg-green-700"
                        >
                          Guardar permisos
                        </button>
                      </div>
